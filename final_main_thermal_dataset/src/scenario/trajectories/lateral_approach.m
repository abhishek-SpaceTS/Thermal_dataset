function [positions, velocities] = lateral_approach(d_start, d_end, frames, dt, config)
% Configurable randomness parameters
    ...
    % Lateral motion is along x, so it is bounded by the HORIZONTAL limit.
    % framing_limits also reserves the target's angular radius, so the body
    % stays inside the frame instead of straddling the edge.
    lim_start = framing_limits(config, true_d_start);
    lim_end   = framing_limits(config, true_d_end);
    start_offset = true_d_start * lim_start;
    end_offset   = true_d_end   * lim_end;
    pos_start = [start_offset; 0; true_d_start];
    pos_end   = [-end_offset; 0; true_d_end];
